   ...
   byte = fifo_s_get(p_obj->data_fifo);	
    ...
    switch(p_obj->unpack_step)			
    {
      ...
      case STEP_LENGTH_HIGH:	
						{
							...
							if(p_obj->data_len < (200 - sizeof(frame_header_t) - 2))		
							{
      ...
      case STEP_HEADER_CRC8:	
						{
							...
							if (p_obj->index == sizeof(frame_header_t))
							{
								if ( Verify_CRC8_Check_Sum(p_obj->protocol_packet, sizeof(frame_header_t)) )	
								{
      ...
      case STEP_DATA_CRC16:		
						{
						 if (p_obj->index < (sizeof(frame_header_t) + 2 + p_obj->data_len + 2))		
							{
								p_obj->protocol_packet[p_obj->index++] = byte;  		
						 }
							if (p_obj->index >= (sizeof(frame_header_t) + 2 + p_obj->data_len + 2))	
							{
							 ...
							 if ( Verify_CRC16_Check_Sum(p_obj->protocol_packet, sizeof(frame_header_t) + 2 + p_obj->data_len + 2) )	
								{
...
{
  memset(tx_buf, 0, (sizeof(frame_header_t) + 2 + sizeof(client_custom_data_t) + 2));
  static uint8_t seq;
  
  uint16_t frame_length = sizeof(frame_header_t) + 2 + len + 2;	  
  ...
  memcpy(&tx_buf[sizeof(frame_header_t)], (uint8_t*)&cmd_id, 2);		
  Append_CRC8_Check_Sum(tx_buf, sizeof(frame_header_t));		
	
  memcpy(&tx_buf[sizeof(frame_header_t) + 2], p_data, len);			
...
void client_show_data_send(float data1, float data2, float data3, uint8_t mask)
{
	...
	uint8_t 						tx_buf[(sizeof(frame_header_t) + 2 + sizeof(client_custom_data_t) + 2)] = {0};
	uint16_t 						i 										= 0;
	...
	protocol_packet_pack(ROBO_CONNECTION_DATA, (uint8_t *)&updata, sizeof(client_custom_data_t), 0xA5, tx_buf);	


	for(i = 0; i < (sizeof(frame_header_t) + 2 + sizeof(client_custom_data_t) + 2); i++)
	{
		
		USART_SendData(((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4800)), tx_buf[i]);
		while((((USART_TypeDef *) (((uint32_t)0x40000000) + 0x4800))->SR&0X40)==0);
	}
}

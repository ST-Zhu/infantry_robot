...
#line 2 "..\\HAL\\cloudmotor.c"


#line 13 "..\\HAL\\cloudmotor.c"
...
{
	angle->back_flag = 0;
	t_angle.pitch_first = real_angle.pitch;
	t_angle.yaw_first = real_angle.yaw;
	...
	angle->pitch_set = real_angle.pitch;
	angle->pitch_target = -100.0f + (angle->pitch_first - GMPitchEncoder.init_angle);
	angle->yaw_target = -119.30f + (angle->yaw_first - GMYawEncoder.init_angle);
	
	chassis_set.YAW_CENTRE = -119.30f;
	chassis_set.YAW_CENTRE_Init = -119.30f;
	chassis_set.chassis_cmd_slow_set_follow.input = chassis_set.chassis_cmd_slow_set_follow.out = -119.30f;
...
void angle_set_update(struct Angle *angle)
{
	...
	if(angle->back_flag == 0)
	{
		if(((int)angle->yaw_set != (int)angle->yaw_target) || ((int)angle->pitch_set != (int)angle->pitch_target))
		{
			if(((int)angle->yaw_set != (int)angle->yaw_target))
				angle->yaw_set = angle->yaw_set < angle->yaw_target ? angle->yaw_set + 0.01f : angle->yaw_set - 0.01f;
			if(((int)angle->pitch_set != (int)angle->pitch_target))
			{
				angle->pitch_set = angle->pitch_set < angle->pitch_target ? angle->pitch_set + 0.01f : angle->pitch_set - 0.01f;
	...
	{			
		if(rc.mouse_r == 1)
		{
			...
			angle->yaw_set += rc.L_x / 3000.0f - (rc.mouse_x / 300.0f) + out[12];
			
			
			angle->pitch_remote_last = angle->pitch_remote_now;
			angle->pitch_remote_now += (-rc.L_y / 4000.0f) + (-rc.mouse_y / 400.0f) + out[13] ;
			angle->pitch_differ = angle->pitch_remote_now - angle->pitch_remote_last;
		}

		else if(rc.mouse_r == 0)
		{ 
		...
		if(GMPitchEncoder.ecd_angle <= -107.0f && angle->pitch_differ > 0.0f)
			angle->pitch_set +=  (angle->pitch_differ);
		 
		if(GMPitchEncoder.ecd_angle >= -94.0f && angle->pitch_differ < 0.0f)
			angle->pitch_set +=  (angle->pitch_differ);

		if(GMPitchEncoder.ecd_angle > -107.0f && GMPitchEncoder.ecd_angle < -94.0f)
			angle->pitch_set +=  angle->pitch_differ;	 
	}
...
void angle_out_update(struct Angle *angle)
{
	...
	out[6] = Calculate_Current_Value(&pid[6], angle->yaw_set, real_angle.yaw);		
	out[7] = Calculate_Current_Value(&pid[7], out[6], real_angle.gz);
	
	out[4] = Calculate_Current_Value(&pid[4], angle->pitch_set, real_angle.pitch);
	out[5] = Calculate_Current_Value(&pid[5], out[4], real_angle.gx);
	
	if(rc.off_line_flag == 0)
		Set_CloudMotor_Current((int16_t)(out[7]),-(int16_t)(out[5]));

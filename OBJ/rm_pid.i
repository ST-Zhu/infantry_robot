...
void ALL_Pid_Incr_Configuration()
{
	...
	pid_incr_init(&pid_incr[0], PID_INCR_CHASSIS, 5000.0f, 50.0f);
	pid_incr_init(&pid_incr[1], PID_INCR_CHASSIS, 5000.0f, 50.0f);
	pid_incr_init(&pid_incr[2], PID_INCR_CHASSIS, 5000.0f, 50.0f);
	pid_incr_init(&pid_incr[3], PID_INCR_CHASSIS, 5000.0f, 50.0f);
...
#line 148 "..\\BSP\\rm_pid.c"



void All_Pid_Configuration(PID pid[])
{	
	pid[4].f_pid_init = pid_init;
	pid[4].f_pid_init(&pid[4], 50.0f, 0.0f, 0.0f, 250.0f, 50.0f, 100.0f, 250.0f);
	
	pid[5].f_pid_init = pid_init;
	pid[5].f_pid_init(&pid[5], 45.0f, 1.0f, 80.0f, 5000.0f, 500.0f, 1000.0f, 5000.0f);

	pid[6].f_pid_init = pid_init;
	pid[6].f_pid_init(&pid[6], 25.0f, 0.0f, 0.0f, 1000.0f, 50.0f, 500.0f, 1000.0f);

	pid[7].f_pid_init = pid_init;
	pid[7].f_pid_init(&pid[7], 320.0f, 4.0f, 80.0f, 30000.0f, 1000.0f, 10000.0f, 30000.0f);
	
	pid[8].f_pid_init = pid_init;
	pid[8].f_pid_init(&pid[8], 10.0f, 0.0f, 30.0f, 500.0f, 0.0f, 500.0f, 500.0f);
	
	pid[9].f_pid_init = pid_init;
	pid[9].f_pid_init(&pid[9], 80.0f, 0.0f, 0.0f, 5000.0f, 0.0f, 0.0f, 5000.0f);
	
	pid[10].f_pid_init = pid_init;
	pid[10].f_pid_init(&pid[10], 12.5f, 0.0f, 100.0f, 5000.0f, 0.0f, 5000.0f, 5000.0f);
	
	pid[12].f_pid_init = pid_init;
	pid[12].f_pid_init(&pid[12], 0.005f, 0.0f, 0.0001f, 1.5f, 0.0f, 0.2f, 1.7f);
	
	pid[13].f_pid_init = pid_init;
	pid[13].f_pid_init(&pid[13], 0.005f, 0.0f, 0.0001f, 1.5f, 0.0f, 0.2f, 1.7f);
	
	pid[14].f_pid_init = pid_init;
	pid[14].f_pid_init(&pid[14], 5.0f, 0.0f, 0.0f, 50.0f, 0.0f, 0.0f, 50.0f);
	 
	pid[15].f_pid_init = pid_init;
	pid[15].f_pid_init(&pid[15], 5.0f, 0.0f, 0.0f, 50.0f, 0.0f, 0.0f, 50.0f);
	
	pid[16].f_pid_init = pid_init;
	pid[16].f_pid_init(&pid[16], 50.0f, 0.0f, 0.0f, 15000.0f, 500.0f, 15000.0f, 15000.0f);
}

...
void judgement_data_handler(uint8_t *p_frame)
{
  ...
  memcpy(p_header, p_frame, sizeof(frame_header_t));

  uint16_t data_length = p_header->data_length;		
  uint16_t cmd_id      = *(uint16_t *)(p_frame + sizeof(frame_header_t)); 
  uint8_t *data_addr   = p_frame + sizeof(frame_header_t) + 2;	
	...
	
  {
	...
	case COURT_EVENT_DATA_ID:
              memcpy(&judge_rece_mesg.court_event_data, data_addr, data_length);
	            GPIO_ResetBits(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x1800)),((uint16_t)0x0008));
		          break;

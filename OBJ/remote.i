...
void Remote_uart1_init(void)
{
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	GPIO_PinAFConfig(((GPIO_TypeDef *) ((((uint32_t)0x40000000) + 0x00020000) + 0x0400)), ((uint8_t)0x07), ((uint8_t)0x07)); 
  ...
  GPIO_InitStructure.GPIO_Pin = ((uint16_t)0x0080); 
	...
	USART_DeInit(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000)));
	...
	USART_InitStructure.USART_Mode = ((uint16_t)0x0004);	
  ...
  USART_DMACmd(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000)), ((uint16_t)0x0040), ENABLE);  
  ...
  DMA_InitStructure.DMA_Channel = ((uint32_t)0x08000000);  
  DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&(((USART_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x1000))->DR);
  DMA_InitStructure.DMA_Memory0BaseAddr = (u32)RXBuff;
...
{
	if(DMA_GetITStatus(((DMA_Stream_TypeDef *) (((((uint32_t)0x40000000) + 0x00020000) + 0x6400) + 0x040)), ((uint32_t)0x10208000)))
	{
		DMA_ClearFlag(((DMA_Stream_TypeDef *) (((((uint32_t)0x40000000) + 0x00020000) + 0x6400) + 0x040)), ((uint32_t)0x10200000));
		DMA_ClearITPendingBit(((DMA_Stream_TypeDef *) (((((uint32_t)0x40000000) + 0x00020000) + 0x6400) + 0x040)), ((uint32_t)0x10208000));
		
		rc.cnts++;
		
		rc.R_x = 1024 - ((RXBuff[0]| (RXBuff[1] << 8)) & 0x07ff); 
		rc.R_y = 1024 - (((RXBuff[1] >> 3) | (RXBuff[2] << 5)) & 0x07ff); 
		rc.L_x = 1024 - (((RXBuff[2] >> 6) | (RXBuff[3] << 2) | (RXBuff[4] << 10))& 0x07ff);
		rc.L_y = 1024 - (((RXBuff[4] >> 1) | (RXBuff[5] << 7)) & 0x07ff); 
		rc.sl = ((RXBuff[5] >> 4)& 0x000C) >> 2; 
		rc.sr = ((RXBuff[5] >> 4)& 0x0003); 
		
		rc.mouse_x = RXBuff[6] | (RXBuff[7] << 8); 
		rc.mouse_y = RXBuff[8] | (RXBuff[9] << 8); 
		rc.mouse_z = RXBuff[10] | (RXBuff[11] << 8); 
		rc.mouse_l = RXBuff[12]; 
		rc.mouse_r = RXBuff[13]; 
		rc.key = RXBuff[14] | (RXBuff[15] << 8); 
		rc.W = (rc.key&0x01)>>0;
		rc.S = (rc.key&0x02)>>1;
		rc.A = (rc.key&0x04)>>2;
		rc.D = (rc.key&0x08)>>3;

...
Temperature temperature = {0.0f, 0.0f, 40.0f};
...
void temperature_ADC_init(void)
{
    ...
    ADC_CommonInitStructure.ADC_Prescaler = ((uint32_t)0x00010000);
    ADC_CommonInit(&ADC_CommonInitStructure);
    temperature_ADC_Reset();

    ADC_RegularChannelConfig(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)), ((uint8_t)0x12), 1, ((uint8_t)0x01));
...
uint16_t get_ADC(uint8_t ch)
{
    ...
    ADC_ClearFlag(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)),((uint8_t)0x10)|((uint8_t)0x20)|((uint8_t)0x02));
    ADC_RegularChannelConfig(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)), ch, 1, ((uint8_t)0x01));

    ADC_SoftwareStartConv(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)));

    while (!ADC_GetFlagStatus(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)), ((uint8_t)0x02)))
    {
        ;
    }
    return ADC_GetConversionValue(((ADC_TypeDef *) ((((uint32_t)0x40000000) + 0x00010000) + 0x2000)));
}
u8 get_set_temp(void)
{
	temperature.temp_set = (int8_t)(get_temprate()) + 10;
	if (temperature.temp_set > (int8_t)(temperature.temp_max_set))
	{
			temperature.temp_set = (int8_t)(temperature.temp_max_set);
	}
	if(temperature.temp_set < 0)
	{
		temperature.temp_set = 0;
	}
	if((int8_t)(temperature.temp_set)  == (int8_t)(0.0f))
		return 0;
	else 
		return 1;

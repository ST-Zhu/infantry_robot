...
#line 2 "..\\HAL\\feedmotor.c"

Feed_set feed_set = {0};
Feed_Motor_State feed_state = {0};

void Shoot()
{
	feed_state.heat_control_state = shooter_heat_control();
	if(feed_state.heat_control_state == 1 || rc.E == 1)
	{
			feed_state.now_mouse_l = (rc.mouse_l == 1) || (rc.sl == 2) || (feed_state.shoot_plan == 1);
			if(feed_state.now_mouse_l == 1 )
			{
				feed_state.pressState = 2;
				feed_set.feed_speed_set = 400;
			}
			else if(feed_state.now_mouse_l == 0 )
			{
			...
			if(feed_state.last_Jam == 0 && feed_state.now_Jam == 1)
			{
				...
				feed_set.feed_angle_set = feed_set.feed_angle_real - 2160;
	...
	switch(feed_state.pressState)
	{
		...
		case 2 : 
			if(feed_state.press_flag != 1)
			{
				...
				feed_state.dance_time ++;
				if(feed_state.dance_time > 200)
				{
		...
		case 0 : 
			...
			else
			{
				...
				feed_set.feed_angle_set = ((feed_state.sum_angle / 2160) + 1) * 2160 + feed_set.feed_angle_and;
				fireMotorDoubleLoopPid_out_update();
			}
			feed_state.dance_time = 0;
...
int16_t IfJam2(int16_t state)
{
	if(state == 1 || state == 0)
	{
		if(fabs(feed_set.feed_angle_real - feed_set.feed_angle_set) >= 100)
	...
	{
		if(fabs(feed_set.feed_speed_real - feed_set.feed_speed_set) >= 200)
			feed_state.jamTime_1ms++;
		else 
			feed_state.jamTime_1ms = 0;
		if(feed_state.jamTime_1ms > 200)
...
void fireMotorSingleLoopPid_out_update(void)
{
	...
	out[9] = Calculate_Current_Value(&pid[9], feed_set.feed_speed_set, feed_set.feed_speed_real);
	Set_FeedMotor_Current((int16_t)out[9]);
}

void fireMotorDoubleLoopPid_out_update(void)
{
	fireMotorPidReal_update();
	out[10] = Calculate_Current_Value(&pid[10], feed_set.feed_angle_set, feed_set.feed_angle_real);
	Set_FeedMotor_Current((int16_t)out[10]);
}

void shoot_plan()
{
	if(*((volatile unsigned long *)((((((((uint32_t)0x40000000) + 0x00020000) + 0x1400)+16) & 0xF0000000)+0x2000000+(((((((uint32_t)0x40000000) + 0x00020000) + 0x1400)+16) &0xFFFFF)<<5)+(0<<2)))) != 0)
	{
		feed_state.shoot_plan_back = 0;
		feed_state.shoot_plan = 1;
		feed_set.feed_angle_and = 0;
	}
	else
	{
		feed_state.shoot_plan_back = 1;
		feed_state.shoot_plan = 0;
		feed_set.feed_angle_and = (int16_t)feed_set.feed_angle_real % 2160;
	}
...
void Set_FeedMotor_Current(int16_t feed_motor_iq)
{
    ...
    CAN_Transmit(((CAN_TypeDef *) (((uint32_t)0x40000000) + 0x6800)),&tx_message);
}

...
#line 2 "..\\HAL\\chassismotor.c"
#line 3 "..\\HAL\\chassismotor.c"

Chassis_set chassis_set = {0, 0, 0, 0, 0, 0, 0, 0};

float max_speed = 0.0f;
unsigned short AccStep[335] = 
{
...
void chassis_init(void)
{
	const static float chassis_x_order_filter[1] = {0.39999999f};
    const static float chassis_y_order_filter[1] = {0.39999999f};
	const static float chassis_follow_order_filter[1] = {0.16666666f};
	const static float chassis_follow_second_order_filter[1] = {0.1666666f};
	const static float chassis_x_second_order_filter[1] = {3.33333333f};
	const static float chassis_y_second_order_filter[1] = {3.39999999f};
	const static float chassis_x_stop_order_filter[1] = {0.33333333f};
	const static float chassis_y_stop_order_filter[1] = {0.39999999f};	
	
		    
  first_order_filter_init(&chassis_set.chassis_cmd_slow_set_vx, 0.001f, chassis_x_order_filter);
  first_order_filter_init(&chassis_set.chassis_cmd_slow_set_vy, 0.001f, chassis_y_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_slow_set_follow, 0.001f, chassis_follow_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_slow_set_follow_second, 0.001f, chassis_follow_second_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_slow_set_second_vx, 0.001f, chassis_x_second_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_slow_set_second_vy, 0.001f, chassis_y_second_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_stop_set_vx, 0.001f, chassis_x_stop_order_filter);
	first_order_filter_init(&chassis_set.chassis_cmd_stop_set_vy, 0.001f, chassis_y_stop_order_filter);
...
void chassis_set_update()
{
	...
	else
	{
		...
		if(GMYawEncoder.ecd_angle < (int16_t)(chassis_set.YAW_CENTRE / 360.0f) * 360.0f + chassis_set.YAW_CENTRE_Init + 180.0f)
			chassis_set.follow_set = (int16_t)(chassis_set.YAW_CENTRE / 360.0f) * 360.0f + chassis_set.YAW_CENTRE_Init;
		else if(GMYawEncoder.ecd_angle >= (int16_t)(chassis_set.YAW_CENTRE / 360.0f) * 360.0f + chassis_set.YAW_CENTRE_Init + 180.0f)
			chassis_set.follow_set = (int16_t)(chassis_set.YAW_CENTRE / 360.0f + 1) * 360.0f + chassis_set.YAW_CENTRE_Init;
	...
	out[8] = Calculate_Current_Value(&pid[8], chassis_set.follow_set, GMYawEncoder.ecd_angle);
	if((int16_t)max_speed <= 755)
		first_order_filter_cali(&chassis_set.chassis_cmd_slow_set_follow,  -out[8], 2);
	else 
		first_order_filter_cali(&chassis_set.chassis_cmd_slow_set_follow_second,  -out[8], 2);
...
void chassis_out_update()
{
	...
	out_incr[0] = pid_incr_calc(&pid_incr[0], chassis_set.cm1_real, chassis_set.cm_set[0]);
	out_incr[1] = pid_incr_calc(&pid_incr[1], chassis_set.cm2_real, chassis_set.cm_set[1]);
	out_incr[2] = pid_incr_calc(&pid_incr[2], chassis_set.cm3_real, chassis_set.cm_set[2]);
	out_incr[3] = pid_incr_calc(&pid_incr[3], chassis_set.cm4_real, chassis_set.cm_set[3]);
	if(chassis_heat_control() == 1)
	{
		chassis_set.chassis_max_current = 15000.0f - Calculate_Current_Value(&pid[16], 60.0f, judge_rece_mesg.power_heat_data.chassis_power_buffer);

	}
	else 
	{
		chassis_set.chassis_max_current = 15000.0f;
	}
	chassis_set.total_current_real = fabs(CM1Encoder.current) + fabs(CM2Encoder.current) + fabs(CM3Encoder.current) + fabs(CM4Encoder.current);
	chassis_set.total_current_set = fabs(out_incr[0]) + fabs(out_incr[1]) + fabs(out_incr[2]) + fabs(out_incr[3]);
	...
	{
		out_fr = out_incr[0] * chassis_set.current_ratio;
		out_fl = out_incr[1] * chassis_set.current_ratio;
		out_bl = out_incr[2] * chassis_set.current_ratio;
		out_br = out_incr[3] * chassis_set.current_ratio;
...
{ 
		...
		ecd_angle_chazhi=-(GMYawEncoder.ecd_angle - chassis_set.YAW_DIR);
		*x=raw_x*cos(ecd_angle_chazhi*0.01745f)+raw_y*sin(ecd_angle_chazhi*0.01745f); 
		*y=raw_y*cos(ecd_angle_chazhi*0.01745f)-raw_x*sin(ecd_angle_chazhi*0.01745f);
}
...
void mecanum_Resolving(Chassis_set *set, int z)
{
	...
	last_rc_x = rc_x;
	rc_x = rc.R_x * 1.595959f;
	rc_y = rc.R_y * 1.59595959f;
	...
	if(chassis_set.y_filted <= 700.0f && chassis_set.y_filted >= -700.0f)
	{
		...
		else if(rc.W == 0 && rc.S == 1)
		{
			rc_y = 1050.0f;
		}
		first_order_filter_cali(&set->chassis_cmd_slow_set_vy, rc_y, 1);
	}
	else 
	{
		state_a = 2;
		...
		first_order_filter_cali(&set->chassis_cmd_slow_set_second_vy, rc_y, 1);
	}
	if(chassis_set.x_filted <= 700.0f && chassis_set.x_filted >= -700.0f)
	{
		...
		else if(rc.A == 0 && rc.D == 1)
		{
			rc_x = -1050.0f;
		}
		first_order_filter_cali(&set->chassis_cmd_slow_set_vx, rc_x, 0);
	}
	else 
	{
		...
		first_order_filter_cali(&set->chassis_cmd_slow_set_second_vx, rc_x, 0);
	}
	...
	if(fabs(rc_y) <= 10.0f)
	{
		...
		first_order_filter_cali(&set->chassis_cmd_stop_set_vy, rc_y, 1);
	}
		
	if(fabs(rc_x) <= 10.0f)
	{

		first_order_filter_cali(&set->chassis_cmd_stop_set_vx, rc_x, 0);
	}
	...
	wheel_speed[3] = Y_filted + X_filted - z;    
...
void Set_ChassisMotor_Current(int16_t cm1_iq, int16_t cm2_iq, int16_t cm3_iq, int16_t cm4_iq)		
{
    ...
    tx_message.IDE = ((uint32_t)0x00000000);
    tx_message.RTR = ((uint32_t)0x00000000);
    tx_message.DLC = 0x08;
    ...
    CAN_Transmit(((CAN_TypeDef *) (((uint32_t)0x40000000) + 0x6400)),&tx_message);
}
